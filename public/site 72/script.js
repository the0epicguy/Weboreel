const heroTitle = document.querySelector('h1');
const blobs = document.querySelectorAll('.blob');

document.addEventListener('mousemove', (e) => {
    const x = e.clientX / window.innerWidth;
    const y = e.clientY / window.innerHeight;

    // Distort text gently
    const skewX = (x - 0.5) * 20;
    const skewY = (y - 0.5) * 10;
    heroTitle.style.transform = `skew(${skewX}deg, ${skewY}deg) scale(${1 + Math.abs(x - 0.5) * 0.2})`;

    // Move blobs towards mouse slightly
    blobs.forEach((blob, index) => {
        const speed = (index + 1) * 0.05;
        const currentTransform = getComputedStyle(blob).transform;
        
        // This is a rough interactive tracking implementation
        blob.style.transform = `translate(${e.clientX * speed}px, ${e.clientY * speed}px)`;
        // Using transform here overriding the animation isn't ideal but gives a chaotic fast response 
        // to mouse over the smooth CSS animation background
    });
});

// Smooth reset on mouse leave window
document.addEventListener('mouseleave', () => {
    heroTitle.style.transform = 'skew(0deg, 0deg) scale(1)';
    blobs.forEach(blob => {
        blob.style.transform = ''; // return to CSS animation control
    });
});
